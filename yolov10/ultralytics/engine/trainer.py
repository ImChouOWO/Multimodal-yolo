...
class BaseTrainer:
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
        ...
        self.trainset, self.testset, self.fusionset = self.get_dataset(self.data)
        self.ema = None
    ...
    def _fusion_process(self, batch):
        ...
        # print(f"fusion_tensor:{batch['fusion_tensor']}")
        return batch
        ...
        while True:
            ...
            for i, batch in pbar:
                ...
                with torch.cuda.amp.autocast(self.amp):
                    ...
                    # task.py 中的loss函數包含執行forward

                    #歸一化
                    batch["fusion_tensor"] = batch["fusion_tensor"].to(self.device, non_blocking=True).float() / 255
                    self.loss, self.loss_items = self.model(batch, batch["fusion_tensor"])
                    if RANK != -1:
    ...
    def get_dataset(data):
        ...
        """
        return data["train"], data.get("val") or data.get("test"), data.get("fusion",None)
...
if __name__ == "__main__":
    test = BaseTrainer(overrides={"data": "/Users/zhouchenghan/python/GPS_IMU/multimodal-yolo/Rock Paper Scissors v14i Yolov11/data.yaml"})

    print(test._setup_train(world_size=1))    
